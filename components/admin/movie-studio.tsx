...
type EditorContext = { contentType: 'movie' | 'series'; episodes: NonNullable<AdminMovie['episodes']>; setEpisodes: (episodes: NonNullable<AdminMovie['episodes']>) => void; cast: AdminMovie['cast']; setCast: (cast: AdminMovie['cast']) => void; downloadStatus: 'available' | 'pending'; setDownloadStatus: (status: 'available' | 'pending') => void };
const ContentTypeContext = createContext<EditorContext>({ contentType: 'movie', episodes: [], setEpisodes: () => undefined, cast: [], setCast: () => undefined, downloadStatus: 'pending', setDownloadStatus: () => undefined });
...
export function MovieStudio({ initialMovies, initialAuditEvents, controls }: { initialMovies: AdminMovie[]; initialAuditEvents: AuditEvent[]; controls: RuntimeControls }) {
  ...
  return <ContentTypeContext.Provider value={{ contentType: draft.contentType ?? 'movie', episodes: draft.episodes ?? [], setEpisodes: (episodes) => update('episodes', episodes), cast: draft.cast, setCast: (cast) => update('cast', cast), downloadStatus: draft.downloadStatus ?? ((draft.downloadSources?.length ?? 0) > 0 ? 'available' : 'pending'), setDownloadStatus: (status) => update('downloadStatus', status) }}><div className="mx-auto max-w-[1500px] px-5 py-8 sm:px-8 lg:px-12">
