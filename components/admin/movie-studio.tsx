...
type EditorContext = { contentType: 'movie' | 'series'; episodes: NonNullable<AdminMovie['episodes']>; setEpisodes: (episodes: NonNullable<AdminMovie['episodes']>) => void; cast: AdminMovie['cast']; setCast: (cast: AdminMovie['cast']) => void; uploadCastImage: (file: File, index: number) => Promise<void>; downloadStatus: 'available' | 'pending'; setDownloadStatus: (status: 'available' | 'pending') => void };
const ContentTypeContext = createContext<EditorContext>({ contentType: 'movie', episodes: [], setEpisodes: () => undefined, cast: [], setCast: () => undefined, uploadCastImage: async () => undefined, downloadStatus: 'pending', setDownloadStatus: () => undefined });
...
export function MovieStudio({ initialMovies, initialAuditEvents, controls }: { initialMovies: AdminMovie[]; initialAuditEvents: AuditEvent[]; controls: RuntimeControls }) {
  ...
  async function upload(file: File | undefined, field: 'poster' | 'backdrop' | 'subtitleUrl' | 'castImage'): Promise<string | undefined> {
    ...
    else { if (field !== 'castImage') update(field, result.path); setMessage({ tone: 'success', text: `${field === 'poster' ? 'Poster' : field === 'backdrop' ? 'Backdrop' : field === 'castImage' ? 'Profile image' : 'Subtitle file'} uploaded.` }); }
    setBusy(false);
    return response?.ok ? result.path : undefined;
  }

  async function uploadCastImage(file: File, index: number) { const path = await upload(file, 'castImage'); if (!path) return; setDraft((current) => ({ ...current, cast: current.cast.map((member, itemIndex) => itemIndex === index ? { ...(typeof member === 'string' ? { actor: member } : member), image: path } : member) })); }

  return <ContentTypeContext.Provider value={{ contentType: draft.contentType ?? 'movie', episodes: draft.episodes ?? [], setEpisodes: (episodes) => update('episodes', episodes), cast: draft.cast, setCast: (cast) => update('cast', cast), uploadCastImage, downloadStatus: draft.downloadStatus ?? ((draft.downloadSources?.length ?? 0) > 0 ? 'available' : 'pending'), setDownloadStatus: (status) => update('downloadStatus', status) }}><div className="mx-auto max-w-[1500px] px-5 py-8 sm:px-8 lg:px-12">
...
function Field({ label, wide, children }: { label: string; wide?: boolean; children: React.ReactNode }) { const context = useContext(ContentTypeContext); const isEpisode = label.startsWith('Series episodes'); const isCast = label.startsWith('Cast'); const isMovieOnly = label.startsWith('Streaming servers') || label.startsWith('Download options'); if ((isEpisode && context.contentType !== 'series') || (isMovieOnly && context.contentType === 'series')) return null; const content = isEpisode ? <EpisodeBuilder episodes={context.episodes} onChange={context.setEpisodes}/> : isCast ? <CastBuilder cast={context.cast} onChange={context.setCast} onUpload={context.uploadCastImage}/> : children; return <label className={wide ? 'md:col-span-2' : ''}><span className="mb-2 block text-xs font-semibold uppercase tracking-[.14em] text-white/40">{label}</span><div className="[&_input]:h-10 [&_input]:border-white/12 [&_input]:bg-black/15 [&_input]:text-white [&_textarea]:border-white/12 [&_textarea]:bg-black/15 [&_textarea]:text-white [&_select]:h-10 [&_select]:border-white/12 [&_select]:bg-black/15 [&_select]:text-white">{content}</div></label>; }
