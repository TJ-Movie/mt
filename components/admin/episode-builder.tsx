  ...
  return (
    <div className="space-y-3">
      {episodes.map((episode, index) => (
        <div
          key={`${index}-${episode.season}-${episode.episode}`}
        ...
        >
          <div className="grid gap-3 md:grid-cols-[100px_100px_1fr_1fr_auto]">
            <label>
              <span className="mb-1 block text-xs text-white/40">Season</span>
              <Input
                type="number"
                min={1}
                value={episode.season}
                onChange={(event) =>
                  update(index, 'season', event.target.value)
                }
              />
            </label>
            <label>
              <span className="mb-1 block text-xs text-white/40">Episode</span>
              <Input
                type="number"
                min={1}
                value={episode.episode}
                onChange={(event) =>
                  update(index, 'episode', event.target.value)
                }
              />
            </label>
            <label>
              <span className="mb-1 block text-xs text-white/40">Title</span>
              <Input
                value={episode.title}
            ...
            <Button
              ...
              onClick={() =>
                onChange(episodes.filter((_, itemIndex) => itemIndex !== index))
              }
              aria-label={`Remove episode ${episode.season}-${episode.episode}`}
            >
              <Trash2 />
            </Button>
          </div>
          <div className="grid gap-3 md:grid-cols-2">
            <ArtworkField
              ...
              onUrlChange={(value) => update(index, 'thumbnail', value)}
              onUpload={(file) => upload(index, 'thumbnail', file)}
            />
            <ArtworkField
              label="Episode Backdrop / Cover"
              value={episode.backdrop ?? ''}
              onUrlChange={(value) => update(index, 'backdrop', value)}
              onUpload={(file) => upload(index, 'backdrop', file)}
            />
            <label>
              <span className="mb-1 block text-xs text-white/40">
                Rating / score
              ...
              <Input
                ...
                onChange={(event) =>
                  update(index, 'rating', event.target.value)
                }
              />
            </label>
            <label>
              ...
              <NativeSelect
                value={
                  ...
                  ((episode.downloadSources?.length ?? 0) > 0
                    ? 'available'
                    : 'pending')
                }
                onChange={(event) =>
            ...
            <label className="md:col-span-2">
              <span className="mb-1 block text-xs text-white/40">
                Download mirrors (one URL per line)
              </span>
              <Textarea
                value={(episode.downloadSources ?? [])
                  .map(
                    (source) =>
                ...
                placeholder="Server 1 | WEB-DL | 1080p | 1.2 GB | https://..."
                className="min-h-20"
              />
            </label>
          </div>
      ...
      <p className="text-xs text-white/40">
        Upload JPG, PNG, or WebP artwork. Files are saved as media paths in the
