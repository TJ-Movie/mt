...
import { ArrowRight, ChevronDown, ChevronLeft, ChevronRight, Globe2, Menu, Play, Search, Sparkles, Star, X } from 'lucide-react';
...
export function MovieBrowser({ movies, adsEnabled }: { movies: PublicMovie[]; adsEnabled: boolean }) {
  ...
  const heroMovies = featuredMovies.length > 0 ? featuredMovies : movies.slice(0, 1);
  const [heroIndex, setHeroIndex] = useState(0);
  const filtered = useMemo(() => movies.filter((movie) => {
    const castNames = movie.cast.map((person) => typeof person === 'string' ? person : `${person.actor} ${person.character ?? ''}`).join(' ');
    const matchesQuery = `${movie.title} ${movie.director} ${castNames}`.toLowerCase().includes(query.trim().toLowerCase());
  ...
  if (!heroMovies.length) return <main className="grid min-h-screen place-items-center bg-[#f2efe9] px-5 text-center text-[#181916]"><div><p className="font-serif text-4xl">Sublyra<span className="text-[#b43a2e]">.</span></p><h1 className="mt-8 font-serif text-5xl tracking-[-.04em]">The next collection is being prepared.</h1><p className="mx-auto mt-4 max-w-lg text-base leading-7 text-black/50">Please return soon for newly curated multilingual cinema.</p></div></main>;
  ...
  return <main className="min-h-screen overflow-hidden bg-[#f2efe9] text-[#181916]">
    ...
    <section id="top" aria-roledescription="carousel" aria-label="Featured content" className="relative min-h-[760px] overflow-hidden pt-20">
      {heroMovies.map((movie, index) => {
        const active = index === heroIndex;
        const href = contentHref(movie);
        return <article key={movie.slug} aria-hidden={!active} aria-label={`${index + 1} of ${heroMovies.length}: ${movie.title}`} className={`absolute inset-0 pt-20 transition-[opacity,transform] duration-1000 ease-out motion-reduce:transition-none ${active ? 'z-10 translate-x-0 opacity-100' : 'pointer-events-none z-0 translate-x-4 opacity-0'}`}>
          <img src={movie.backdrop} alt="" className="absolute inset-0 h-full w-full scale-[1.02] object-cover"/>
          ...
          <div className="relative mx-auto flex min-h-[680px] max-w-[1480px] items-end px-5 pb-24 text-white sm:px-8 lg:items-center lg:px-12 lg:pb-0">
            <div className="max-w-3xl pt-24"><div className="mb-7 flex items-center gap-3 text-xs font-semibold uppercase tracking-[.25em] text-white/65"><span className="h-px w-10 bg-[#de5d4f]"/> {featuredMovies.length ? 'Featured content' : 'Latest release'}</div><a href={href} tabIndex={active ? 0 : -1} className="group inline-block"><h1 className="font-serif text-6xl leading-[.9] tracking-[-.055em] transition-colors group-hover:text-[#f3aaa2] sm:text-8xl lg:text-[7.5rem]">{movie.title}</h1></a><p className="mt-5 font-serif text-xl italic text-white/65 sm:text-2xl">{movie.tagline}</p><div className="mt-7 flex flex-wrap gap-x-5 gap-y-2 text-sm text-white/70"><span className="flex items-center gap-1.5"><Star size={14} className="fill-[#de5d4f] text-[#de5d4f]"/> {movie.rating}</span><span>{movie.year}</span><span>{movie.runtime}</span><span>{movie.genre}</span><span>{movie.languages.length} subtitle languages</span></div><p className="mt-6 max-w-xl text-base leading-7 text-white/65">{movie.description}</p><div className="mt-8 flex flex-wrap gap-3"><a href={href} tabIndex={active ? 0 : -1} className="inline-flex items-center gap-3 rounded-full bg-[#de5d4f] px-6 py-3.5 text-sm font-semibold text-white transition hover:bg-[#c64d40]">Explore this {movie.contentType === 'series' ? 'series' : 'film'} <ArrowRight size={17}/></a><a href="#discover" tabIndex={active ? 0 : -1} className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-black/15 px-6 py-3.5 text-sm font-semibold backdrop-blur"><Search size={16}/> Search all films</a></div></div>
          </div>
        </article>;
      })}
      {heroMovies.length > 1 ? <div className="absolute inset-x-0 bottom-7 z-30 mx-auto flex max-w-[1480px] items-center gap-3 px-5 text-white sm:px-8 lg:px-12">
        <button type="button" onClick={() => setHeroIndex((index) => (index - 1 + heroMovies.length) % heroMovies.length)} className="grid size-11 place-items-center rounded-full border border-white/25 bg-black/25 backdrop-blur transition hover:bg-black/45" aria-label="Previous featured title"><ChevronLeft size={20}/></button>
        <div className="flex items-center gap-2" role="tablist" aria-label="Choose featured title">{heroMovies.map((movie, index) => <button key={movie.slug} type="button" role="tab" aria-selected={index === heroIndex} aria-label={`Show ${movie.title}`} onClick={() => setHeroIndex(index)} className={`h-1.5 rounded-full transition-all ${index === heroIndex ? 'w-10 bg-[#ef796d]' : 'w-5 bg-white/40 hover:bg-white/70'}`}/>)}</div>
        <button type="button" onClick={() => setHeroIndex((index) => (index + 1) % heroMovies.length)} className="grid size-11 place-items-center rounded-full border border-white/25 bg-black/25 backdrop-blur transition hover:bg-black/45" aria-label="Next featured title"><ChevronRight size={20}/></button>
        <span className="ml-1 text-xs font-semibold tracking-[.18em] text-white/60" aria-live="polite">{String(heroIndex + 1).padStart(2, '0')} / {String(heroMovies.length).padStart(2, '0')}</span>
      </div> : null}
