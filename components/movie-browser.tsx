  ...
  return <main className="min-h-screen overflow-hidden bg-[#f2efe9] text-[#181916]">
    <header className="fixed inset-x-0 top-0 z-50 border-b border-black/10 bg-[#f2efe9]/90 backdrop-blur-xl">
      <div className="mx-auto flex h-20 max-w-[1480px] items-center px-5 sm:px-8 lg:px-12">
        ...
        <nav className="mx-auto hidden items-center gap-8 text-sm font-medium text-black/60 lg:flex"><a className="text-black" href="#discover">Discover</a><a href="#collection">Movies</a><a href="#languages">Languages</a><a href="#about">About</a></nav>
      ...
      {menu && <nav className="grid gap-4 border-t border-black/10 px-5 py-5 text-sm lg:hidden"><a href="#discover">Discover</a><a href="#collection">Movies</a><a href="#languages">Languages</a><a href="#about">About</a></nav>}
    </header>
    ...
    <section id="top" className="relative min-h-[760px] pt-20">
      ...
      <div className="relative mx-auto flex min-h-[680px] max-w-[1480px] items-end px-5 pb-16 text-white sm:px-8 lg:items-center lg:px-12 lg:pb-0">
        <div className="max-w-3xl pt-24"><div className="mb-7 flex items-center gap-3 text-[11px] font-semibold uppercase tracking-[.25em] text-white/65"><span className="h-px w-10 bg-[#de5d4f]"/> Featured film</div><h1 className="font-serif text-6xl leading-[.9] tracking-[-.055em] sm:text-8xl lg:text-[7.5rem]">{featured.title}</h1><p className="mt-5 font-serif text-xl italic text-white/65 sm:text-2xl">{featured.tagline}</p><div className="mt-7 flex flex-wrap gap-x-5 gap-y-2 text-sm text-white/70"><span className="flex items-center gap-1.5"><Star size={14} className="fill-[#de5d4f] text-[#de5d4f]"/> {featured.rating}</span><span>{featured.year}</span><span>{featured.runtime}</span><span>{featured.genre}</span><span>{featured.languages.length} subtitle languages</span></div><p className="mt-6 max-w-xl text-sm leading-7 text-white/65 sm:text-base">{featured.description}</p><a href={`/movies/${featured.slug}`} className="mt-8 inline-flex items-center gap-3 rounded-full bg-[#de5d4f] px-6 py-3.5 text-sm font-semibold text-white transition hover:bg-[#c64d40]">Explore this film <ArrowRight size={17}/></a></div>
      </div>
