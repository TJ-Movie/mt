...
export async function verifyPlan(plan, root = 'tmp/media') {
  ...
  for (const item of plan.files) {
    ...
    if (!Number.isSafeInteger(item.id) || item.id < 1 || !['720p','1080p'].includes(item.quality) ||
      ...
      identities.has(identity) || keys.has(item.key) || typeof item.verified !== 'boolean') throw new Error('Invalid or duplicate media binding');
  ...
  if (plan.files.some(f => !f.verified) && staged === 0) throw new Error('Pending assets require a staged video');
  return { total: plan.files.length, staged, verified: plan.files.filter(f => f.verified).length };
}
