...
for (const row of rows) {
  ...
  if (['tt0499549', 'tt1630029'].includes(row.imdb_id)) inspected.push({ id: row.id, title: row.title, imdbId: row.imdb_id, status: parsed.status, sources: next.map((x) => ({ quality: x.quality, r2StorageKey: x.r2StorageKey, r2Bytes: x.r2Bytes })), checks, complete });
