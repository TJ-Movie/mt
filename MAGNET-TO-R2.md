...
`.github/workflows/magnet-to-r2.yml` supports manual dispatch, `repository_dispatch` with event type `magnet-to-r2`, and a daily run at 02:23 UTC (07:53 Sri Lanka). Commit the workflow, runner, tests and lockfile to the default branch to enable it. No workflow is triggered by pull requests, and manual runs on non-default branches are skipped. Dispatch payloads cannot change commands or resource limits.
