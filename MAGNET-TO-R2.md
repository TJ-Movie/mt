...
**Current mode: requested batches only.** The recurring hourly schedule has been removed. Start work with manual dispatch or `repository_dispatch`; each invocation processes up to 20 eligible records within its time budget. Existing in-progress/pending runs are not cancelled by this change. If the time budget is reached, explicitly dispatch a continuation for remaining queued records; do not call the batch complete until every intended record has been checked. Failed records require diagnosis/requeueing. The hourly deployment notes below describe the previous configuration, not the current trigger.

`.github/workflows/magnet-to-r2.yml` supports manual dispatch, `repository_dispatch` with event type `magnet-to-r2`, and hourly runs at minute 23 UTC (minute 53 Sri Lanka). Initial catalogue-fill mode has no three-films-per-day limit. Each run considers up to 20 queued records and only claims another film when enough run time remains. A later hourly run continues the remaining queue. Runs are serialized with `cancel-in-progress: false`, so new schedules do not interrupt active uploads. GitHub may delay scheduled runs; this is not a guaranteed start time. Commit the workflow, runner, tests and lockfile to the default branch to enable it. No workflow is triggered by pull requests, and manual runs on non-default branches are skipped. Dispatch payloads cannot change commands or resource limits.
...
CI handles up to 20 torrents sequentially within its run budget, with a 4 GiB whole-torrent cap and 2 GiB free-disk reserve. This is not a promise of 20 completions per hour: slow torrents can consume the budget sooner. Selection starts disabled until metadata and disk checks finish; only the main MP4 is then selected sequentially. Two cached pieces (each capped at 16 MiB) plus two 8 MiB multipart buffers bound these buffer pools, not the entire process. Torrent traffic is capped at 8 MiB/s download and 512 KiB/s peer upload, with 30 connections. R2 multipart upload is backpressured and is not capped by the peer-upload setting. Hourly processing can consume significantly more GitHub Actions minutes; monitor the repository's usage/billing and restore a lower-frequency schedule after the initial fill.
